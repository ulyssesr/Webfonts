...
if($_POST['ur_webfont_hidden'] == 'Y') {

  // store data

  $name = $_POST['ur_webfont_name'];
  update_option('ur_webfont_name', $name);
  $style = $_POST['ur_webfont_style'];
  update_option('ur_webfont_style', $style);
...
<div class="wrap">
  ...
  <?php echo "<h2>" . __( 'UR Web Fonts Setting', 'ur_webfont_setting' ) . "</h2>"; ?>

  <form name="ur_webfont_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
  <input type="hidden" name="ur_webfont_hidden" value="Y">
  ...
  <textarea name="ur_webfont_name" rows="5" cols="150"><?php echo stripslashes($name); ?></textarea>
  </p>
  ...
  <p class="submit">
  <input type="submit" name="Submit" value="<?php _e('Update Options', 'ur_google_webfont' ) ?>" />
  </p>
